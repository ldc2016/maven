 ...
 */
public class ComplexPausedAbleThreadPoolExecutor extends AbstractExecutorService {

    protected final Logger LOG = LoggerFactory.getLogger(ComplexPausedAbleThreadPoolExecutor.class);

    /********* 自定义可暂定的线程池状态及其实现 start **********/
    private  boolean isPaused;
    private ReentrantLock pauseLock = new ReentrantLock();
    private Condition unPaused = pauseLock.newCondition();
    private boolean isDelayed;
    private long delayTime;
    private int stopWorkerCount = 0;
    ...
    /**
     * worker执行runnable之前会执行该方法的逻辑
     * @param t
     * @param r
     * @param worker
     */
    protected void beforeExecute(Thread t, Runnable r, Worker worker) {
        pauseLock.lock();
        try {
            while (isPaused || worker.isPaused()) {
                LOG.info("===>ComplexPausedAbleThreadPoolExecutor.beforeExecute, current status of the threadPool is paused, the thread :{}, will not run!",t.getId());
                unPaused.await();
            }
            LOG.info("===>ComplexPausedAbleThreadPoolExecutor.beforeExecute, current status of the threadPool is not paused, the thread :{}, will start run!",t.getId());
            ...
            pauseLock.unlock();
            if(isDelayed){
                LOG.info("===>ComplexPausedAbleThreadPoolExecutor.beforeExecute, current thread : {} in the threadPool will run after {} millisecond!",t.getId() ,delayTime);
                try {
    ...
    public int getStopWorkerCount() {
        return this.stopWorkerCount;
    }
    ...
    public String getWorkerThreadIds() {
        String str = "";
        for (Worker worker : workers) {
            str = str.concat(Long.toString(worker.getWorkThreadId()) + ",");
        }
        return str;
    }
    ...
    public String getPausedWorkerThreadStatus(){
        ...
        try{
            for (Worker worker : workers) {
                LOG.info("===>ComplexPausedAbleThreadPoolExecutor.getPausedWorkerThreadStatus, " +
                         " worker thread id :{} <===> isPaused :{}",worker.getWorkThreadId(),worker.isPaused());

                str.append("worker thread id = ")
                   .append(worker.getWorkThreadId()).append("isPaused = ")
                   .append(worker.isPaused()).append("\n");
            }
        }finally{
            pauseLock.unlock();
        }
    ...
    public long setWorkerThreadStopAmount(int maxCount) {
        pauseLock.lock();
        try {
            int alreadyStoppedWorkerThread = 0;
            int stopWorkerCount = 0;
            for (Worker worker : workers) {

                if (alreadyStoppedWorkerThread < maxCount) {
                    worker.setPaused(false);
                } else {
                    stopWorkerCount++;
                    worker.setPaused(true);
                }
                alreadyStoppedWorkerThread ++;
            }
            unPaused.signalAll();
    ...
    /********* 自定义可暂定的线程池状态及其实现 end **********/


    // Public constructors and methods
    public ComplexPausedAbleThreadPoolExecutor(int corePoolSize, int maximumPoolSize, long keepAliveTime, TimeUnit unit,
    ...
    public ComplexPausedAbleThreadPoolExecutor(int corePoolSize, int maximumPoolSize, long keepAliveTime, TimeUnit unit,
        ...
        if (corePoolSize < 0 || maximumPoolSize <= 0 || maximumPoolSize < corePoolSize || keepAliveTime < 0){
            throw new IllegalArgumentException("the parameter : corePoolSize->" + corePoolSize + ", maximumPoolSize->"
                    + maximumPoolSize +", keepAliveTime->{} " + keepAliveTime
                    + "of the ComplexPausedAbleThreadPoolExecutor constructor is illegal!");
        }

        if (workQueue == null || threadFactory == null || handler == null){
            throw new IllegalArgumentException("the parameter : workQueue->" + JSON.toJSONString(workQueue)
                    + ", threadFactory->" + JSON.toJSONString(threadFactory)
                    + ", handler-> " + JSON.toJSONString(handler)
                    + " of the ComplexPausedAbleThreadPoolExecutor constructor is illegal!");
        }
    ...
    public ComplexPausedAbleThreadPoolExecutor(int corePoolSize, int maximumPoolSize, long keepAliveTime, TimeUnit unit,
                                               BlockingQueue<Runnable> workQueue, ThreadFactory threadFactory, RejectedExecutionHandler handler,boolean isDelayed,long delayTime) {
        if (corePoolSize < 0 || maximumPoolSize <= 0 || maximumPoolSize < corePoolSize || keepAliveTime < 0)
            throw new IllegalArgumentException("the parameter : corePoolSize->" + corePoolSize + ", maximumPoolSize->"
                    + maximumPoolSize +", keepAliveTime->{} " + keepAliveTime
                    + "of the ComplexPausedAbleThreadPoolExecutor constructor is illegal!");
    ...
    private final class Worker extends AbstractQueuedSynchronizer implements Runnable {
        ...
        /** current work thread id*/
        private long workThreadId;
        /** current work thread status is paused*/
        ...
        Worker(Runnable firstTask) {
            ...
            this.workThreadId = thread.getId();
        }
